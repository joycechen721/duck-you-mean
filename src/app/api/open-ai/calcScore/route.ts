...
export async function POST(req: any) {
    try {
        ...
        const evaluationResponse = await openai.chat.completions.create({
            model: "gpt-3.5-turbo",
            messages: [
                { role: "system", content: "You are an AI evaluator that assigns a score from 1-100 based on how well the user's response answers the given question, in the context of the given subject. If the answer is not in relation to the subject at all, then give a score of 1. Consider relevance, clarity, and completeness. Allow room for improvement with subsequent attempts. Return only the number, with no extra text." },
